...
> **2026-08-29 WebDAV 任务补充**：WebDAV 上传、普通下载和指定备份下载已接入 `DataSyncManager`，任务中心显示阶段/进度、历史、取消和清理；同步操作不再只由设置页持有临时 `Task`。`WebDAVClient` 增加 HTTP(S) URL 校验、请求/资源超时、取消检查和 WebDAV `Depth`/`Content-Type` 请求头；仍未接入翻译和 WebDAV 漫画库迁移等全部 Flutter 后台任务；本地 `.venera` 导入/导出已接入任务中心，本地漫画 `.venera_comics` 已支持跨平台单/多本归档与导入，但仍缺少统一批量导出任务 UI。
> **2026-08-29 源目录并发补充**：目录抓取、可用更新列表和批量源更新的共享状态已增加锁保护，减少源市场刷新与任务中心并发读取时的数据竞争；仍需真实网络源和真机长时间验证。
> **2026-08-29 源更新补充**：在线源目录“Update All”已接入可持久化摘要任务、逐源进度/失败状态和取消，任务中心新增源更新类别；仍未覆盖翻译和 WebDAV 漫画库迁移等全部 Flutter 后台任务，本地漫画批量导入导出仍不完整。
...
| 双页阅读与跨页切分 | 🚧 | 已有双页 spread 基础 UI，章节末评论页可作为独立末尾页进入；真实跨页切分、封面页和 iPad/横屏仍需验收 |
...
| 阅读器内章节评论 | 🚧 | 已加入章节评论 API、独立分页/回复/发送入口；单页画廊模式已加入章节末评论页，双页模式和前后章节哨兵仍需对齐 |
...
| 本地漫画导入导出（CBZ/ZIP/文件夹/ComicInfo.xml） | 🚧 | 已支持 CBZ/ZIP/文件夹和 `.venera_comics` 导入，以及单本 CBZ/`.venera_comics` 导出；Flutter 的 PDF/EPUB、多选批量导出、合并 `.venera_comics` 导出任务 UI 尚未对齐 |
| 图片收藏 / 合集 / 任务中心 | 🚧 | 图片收藏已支持阅读器入口、画册、详情、分享、多选批删与自动刷新；合集可创建/编辑/排序/删除；任务中心现已统一展示下载、迁移、追更、源更新、WebDAV 同步任务并支持取消/历史清理，但翻译、WebDAV 漫画库迁移和后台恢复仍未全部接入；本地整库 `.venera` 导出已纳入任务中心，CBZ/`.venera_comics` 批量导出任务 UI、PDF/EPUB 仍有缺口 |

## M5 同步与系统集成 🚧（同步主链路与深链代码已落地，端到端/指南/iPad 专项仍缺）
| 功能 | 状态 | 备注 |
|---|---|---|
| WebDAV 客户端（PROPFIND/GET/PUT/MKCOL/Basic Auth） | ✅ | |
