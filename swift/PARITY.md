...
> **2026-09-04 上游 v2.2.12→v2.3.0 基线追赶**：移植阅读器 5 项变更中的 3 项——页码指示可点开跳页对话框（显示总页数，输入自动钳制到 1...maxPage，对齐 #57a07e31+#0bed1f2e）、连续模式显式跳页（滑杆/对话框）联动分页器 scrollTo 精确落在目标页（对齐 #681334b9；Swift 每页 frame 固定为视口尺寸、偏移不随图片加载漂移，无需 Flutter 的 repivot 机制，改为消费式跳转请求 `continuousJumpTargetItemID`）。其余 2 项判定 N/A：滑杆可达首末页（#6452169d，Swift 用系统 `Slider` 天然钳制在范围内且 set 去重，模型层首末可达性由跳页钳制测试覆盖）、画廊设置按生效模式显示（#ae214a7b，Flutter 设置页支持每部漫画/设备作用域故需读生效值，Swift 设置页为纯全局作用域、门控语义本就正确）。
> **2026-09-04 隐藏重复章节补全**（对齐 v2.2.12 #bb27c447 的阅读器半边 + 其前置功能）：Swift 此前整体未实现该功能，现已全量落地——`ChapterDuplicates` 模块（组内标题 trim 比较、组间独立、空白标题跳过；步进受限搜索不越过组边界）、每部漫画设备本地开关（implicitData，不随备份同步）、详情页章节区开关（仅存在重复时显示，带数量 toast）与列表过滤（平铺索引保持原值）、阅读器翻章/章节抽屉/连续模式预取统一经 `nextVisibleChapter` 跳过折叠条目。原版另一半（追更对话框不可关闭）Swift 为页面内联状态无模态，维持 N/A。
> **2026-09-04 预翻译任务接入任务中心**（对齐 pre\_translation\_tasks.dart 的任务中心语义）：`PreTranslationTaskManager` 落地——详情页菜单选章（需已开启图片翻译）→ 后台任务逐章解析页列表、逐页经 `ImageDownloader` 取图并调用 `ImageTranslationService.translate`（cacheKey 与阅读器 `translationCacheKey` 完全一致，预译结果阅读时即时命中）；任务中心展示章节加权进度/失败页数/当前章节，支持取消、失败页精确重试与历史清理；持久化 UserDefaults（重载时运行中任务标记取消）。与 Flutter 的差距：无组并发/阶段面板/失败页跨重启恢复，单并发任务。

> **2026-09-04 实时活动视觉与下载接入**：新增 `VeneraXLiveActivity` Widget Extension；采用“封面/内容识别 + 圆环进度”方案，预翻译与下载分别维护活动，下载聚合当前队列并显示队列数量；锁屏/紧凑/最小/展开状态已实现，点击通过 `venera://tasks` 回到任务中心。
> **2026-09-04 M4 预翻译任务恢复与并发收口**：运行中任务不再在重启后伪装成取消；按章节的 `done+failed` 连续前缀作为 checkpoint，源加载完成后自动续跑，暂停状态跨启动保留，任务中心支持暂停/继续/取消。修复章节页列表失败被误记成“第 0 页失败”导致重试只处理首页的问题；页列表变化时安全重扫，`noText` 计为成功。页面按 `min(imageConcurrency, llmConcurrency)`（上限 4）分组并发、整组有序提交，最多重做一个小批次；进度持久化节流至约 1 秒、UI 通知约 4 Hz。
...
## M4 收藏·历史·本地库 ✅（下载、本地库、图片收藏、合集与任务中心闭环已落地）
...
| 本地漫画导入导出（CBZ/ZIP/文件夹/ComicInfo.xml）          | ✅  | 已支持 CBZ/ZIP/文件夹、`.venera_comics`（含多选合并单文件，manifest+comics 布局对齐 Flutter）、PDF/EPUB 与多选导出任务；此前记录「合并单文件缺失」为过时信息                |
| 图片收藏 / 合集 / 任务中心                             | ✅  | 任务中心展示下载、迁移、追更、源更新、WebDAV 同步、WebDAV 漫画库迁移、本地批量导出与预翻译；预翻译具备 checkpoint 自动续跑、跨启动暂停、暂停/继续/取消、失败页精确重试、页列表失败整章重试、受设置约束的分组并发与当前批次页码进度 |
