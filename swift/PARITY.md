...
> **2026-09-03 上游 v2.2.11→v2.2.12 基线追赶**：移植 fork 上游 8 项提交中的 4 项行为修复——归档下载遵循代理/忽略坏证书并可随暂停取消（`HTTPClient.makeDownloadSession`）、连续模式页面帧改用 GeometryReader 实时视口（替代 `UIScreen.main.bounds`，对齐 #252）、源市场描述不再截断 2 行、合集编辑弹窗在「分部标签」布局下可为每个成员命名 tab 且加入合集时未命名不冻结标题。其余 3 项判定 N/A：追更对话框（Swift 为页面内联状态无模态）、隐藏重复章节（Swift 整体未实现该功能）、插件更新源仓库同步（Swift 无多仓库 provenance 体系）；滚动中解码（#260）Swift 预取本就只下载字节、解码由懒加载视图触发，已是修复后行为。
